"""
Step 15: train on original-condition passages, evaluate on original and rewritten conditions.

Run from repository root after Step 14:

    python scripts/16_run_original_to_rewritten_degradation.py
...
PREDICTIONS = RESULTS / "original_to_rewritten_predictions.csv"
METRICS = META / "original_to_rewritten_metrics.csv"
DEGRADATION = META / "original_to_rewritten_degradation_summary.csv"
CONFUSION = META / "original_to_rewritten_confusion_matrices.csv"
CLASS_REPORT = META / "original_to_rewritten_classification_report.csv"
MANIFEST = META / "original_to_rewritten_manifest.csv"
REPORT = LOGS / "step_15_original_to_rewritten_degradation_report.md"
...
def main() -> int:
    ...
    if len(train) != 252:
        print(f"Expected 252 train/original rows, found {len(train)}")
        return 1
    ...
    prediction_rows, metric_rows, confusion_all, class_report_all = [], [], [], []
    for model_name in MODELS:
        for split in EVAL_SPLITS:
            for condition in CONDITIONS:
                ...
                metric_rows.append({"model": model_name, "split": split, "condition": condition, **metrics, "rows": len(y_eval)})
    ...
    # condition-wise loss against original within each model/split
    ...
    write_csv(METRICS, metric_rows, ["model", "split", "condition", "accuracy", "macro_f1", "weighted_f1", "rows"])
    write_csv(DEGRADATION, degradation_rows, ["model", "split", "condition", "baseline_condition", "accuracy", "macro_f1", "weighted_f1", "accuracy_loss_vs_original", "macro_f1_loss_vs_original", "weighted_f1_loss_vs_original", "rows"])
    ...
    artifacts = [PREDICTIONS, METRICS, DEGRADATION, CONFUSION, CLASS_REPORT]
    ...
    test_degradation = [row for row in degradation_rows if row["split"] == "test" and row["model"] == model]
    ...
    REPORT.write_text(
        ...
        "## Test degradation for best original-test model\n\n" + "".join(
            f"- {row['condition']}: macro_f1={row['macro_f1']}, macro_f1_loss={row['macro_f1_loss_vs_original']}\n"
            for row in test_degradation
        ),
        encoding="utf-8",
    )
    print("Ran original-to-rewritten degradation experiment for 3 models.")
    print(f"Best original-test model: {model}; original test macro F1={best_original_test['macro_f1']}")
    return 0
