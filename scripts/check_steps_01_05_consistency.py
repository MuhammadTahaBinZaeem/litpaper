"""
Automated consistency checker for Steps 1-5.

This extends the Step 1-4 checker with Step 5 cleaning-validation outputs.
Run from repository root:

    python scripts/check_steps_01_05_consistency.py
...
def main() -> int:
    ...
    check(len(validation_metrics) == EXPECTED_VALIDATED_SOURCES, "cleaning_validation_metrics row count mismatch", failures)
    check(len(author_validation) == 6, "author_style_validation_summary author count mismatch", failures)
    ...
    check(len(eligible) == EXPECTED_ELIGIBLE_VALIDATED_SOURCES, "eligible validation source count mismatch", failures)
    check(len(excluded) == EXPECTED_EXCLUDED_TRACKED_SOURCES, "excluded validation source count mismatch", failures)
    check(len(flagged) == EXPECTED_VALIDATION_FLAGGED_SOURCES, "validation flagged source count mismatch", failures)
    ...
    print("PASS: Steps 1-5 consistency checks passed.")
    return 0
